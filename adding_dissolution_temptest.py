...
import pandas as pd
from modular_functions_clean_redox import timestep,surface_T,stdrk4
import sklearn
from sklearn.neighbors import KNeighborsRegressor

#%% THIS SCRIPT SUPERCEDES MELTING CALCULATIONS IN MAIN MODEL RUNS - FEED RESULTS FROM adding_dissolution INTO STARTING CONDITIONS FOR MAIN RUNS
#For given parameter combination, run adding_dissolution, followed by ext_1line.py to extract final state of system after surface melting ended
#NOTE: FOR tstart >= 1.5 Gyr AND gwater_GEL >= 700 m RUNS MAY TAKE SEVERAL DAYS TO COMPLETE 
#FINAL MODEL RUN OUTPUT LIKELY TO EXCEED 20GB

#%% Initial setup of arrays and parameters
results = np.zeros(13)

P_final = 90.e5

# frac_volc_arr = np.array([0.1])#,0.5,0.9,1.0]) #fraction of CO2 atmosphere produced through volcanism after surface melting has ended
# tstart_arr = np.array([0.5])#,1.5,3.]) #Gyr
# GEL_arr = np.array([500.]) #m Global Equivalent Layer (GEL) water present at end of habitable era  np.array([100.,300.,500.,700.,1000.])

gwater_GEL = 500. #m GEL
t_start = 0.5 #Gyr
mCO2tot = 1000e-6
frac_volc = 1.
...
q = 150e-3 #crustal heatflow, Wm^-2
phi0 = random.uniform(0.01,0.2) #uncompressed porosity of Venus crust
P_final = 93.e5 #Final atmospheric pressure Pa
g = 8.87 #Venus gravity m s^-2
rho = 2800. #crust density kg m^-3
T = 1000. + 273. #melt temperature at eruption, K
epsilon = 0.125
T_atm = 300. #thermosphere temperature, K
dzdt_pres = 1e9 #m GEL volcanism per Gyr


#%%
secingyr = 1.e9 * (365.*60.*60.*24.) #s in 1 Gyr
...
# For high pressures (reached less often), 1D interpolation between datapoints
...
#For low pressures (reached more often), KNeighborsRegressor provides smooth function for interpolating between data quickly
...
degas = 0 #no volcanic degassing during surface melt layer
magma_yn = 1.
...
# for i4 in range(0,np.size(tstart_arr)):
...
# for i3 in range(3,np.size(frac_volc_arr)):
# if gwater_GEL == 700:
#     if i3 < 2:
#         i3 = int(2)
...
# Set up time and solar luminosity arrays
...
T_surf = np.zeros(nsteps) #surface temperature, K
z_meltlayer = np.zeros(nsteps) #thickness of melt layer, m

Rmm_atm = np.zeros(nsteps) # Relative molecular mass of the atmosphere
Rmm_atm[0] = (moles_CO2[0]*Rmm_CO2 + moles_H2O[0]*Rmm_H2O + moles_O2[0]*Rmm_O2)/moles_atm[0]
Pres[0] = moles_atm[0] * Rmm_atm[0] * g

dz = z[1]-z[0] #depth step size, m

T_arr = T_bg*np.ones(nz)  # array of subsurface temperatures (K) at each depth step
T_arr = T_arr + q*z/k 

fig1, ax1 = plt.subplots()

ax1.plot(T_arr,z/1000)
# ax1.invert_xaxis()
ax1.invert_yaxis()
...
meltfrac = np.zeros(nz) # Melt fraction at each depth
...
z_meltlayer_tot = np.zeros(nsteps) #cumulative melt layer thickness over full model run
moles_O2_esc = np.zeros(nsteps) #moles O2 produced by loss of H from H2O to space

moles_diss = np.zeros(nsteps) #moles H2O dissolved in melt at each timestep
delta_moles_diss_H2O = np.zeros(nsteps) #change in moles dissolved H2O at each timestep
...
for i1 in range(1,nsteps-1):
    ...
    # Terminate surface melting timestep sequence when surface has fallen below basalt solidus
    ...
    #Update surface temperature and temperature-depth array
    ...
    if endmagma == 0:
        ...
        #Find melt fraction at each depth using function defined by fitting to alphaMELTS results
        a = np.array([(P_z+Pres[i1])/1.e5,T_arr])
        b = np.fliplr(np.rot90(a,axes=(1,0)))
        melt_arr_new = (knn.predict(b))
        
        #At high pressures, use weighted average to find melt fraction using data for melting at 10kbar and 15kbar 
        ...
        #Find change in melt layer thickness between this and previous timestep
        for i2 in range(0,nz-1):
            if melt_arr_cum[i2]<melt_arr_new[i2]:
                melt_arr_cumdiff[i2] = melt_arr_new[i2] - melt_arr_cum[i2]
        
        #Update melt layer thickness
        z_meltlayer[i1] = np.mean(melt_arr_new)*z_end
        
        #Calculate maximum moles O2 that can be removed by oxidizing all available melt as a function of depth
        ...
        #%% Water dissolution - to disable set partial_P_H2O = 0 
        
        # Find partial pressure H2O in atmosphere
        partial_P_H2O = (moles_H2O[i1-1]/moles_atm[i1-1])*Pres[i1-1]
        # Find mass fraction H2O in melt
        mass_frac_H2O = 3.44e-8 * ((partial_P_H2O)**0.74)
        
        #Find change in # moles H2O dissolved in melt
        delta_moles_diss_H2O[i1] = mass_frac_H2O*(z_meltlayer[i1] - z_meltlayer[i1-1])*rho/Rmm_H2O
        
        #Error if melt layer thickness has negative value (unphysical)
        if z_meltlayer[i1] < 0:
            print('negative melt layer',z_meltlayer[i1])
            break
        
        #Calculate change in moles H2O in atmosphere
    ...
    #%%
        # Latent heat consumed/released by melting/freezing
    ...
    # Update melt fraction w/ depth array
...
title = ("q50_withdiss_t" + t_string + "_gwater" + gwater_string + "_f_volc" + f_volc_string + ".csv")
...
#%% OPTIONAL PLOTS, EXTREMELY TIME CONSUMING DUE TO ARRAY SIZES
