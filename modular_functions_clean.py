...
#%%
# Plot setup for during surface melt layer
fig1,ax1 = plt.subplots(figsize=(40,50))
ax1.set_ylabel('Depth (km)', fontsize=100,weight='bold')
ax1.set_xlabel('Temperature (K)', fontsize=100,weight='bold')
...
def porespace(tstart,q,phi0):
    ...
    # print('pore closure depth',z_closure)
    # print('GEL',gwater_GEL)
...
def time_evol(tstart,nsteps,metamorphic_degas,frac_volc,P_final,b_err,dzdt_pres,gwater_GEL,T,epsilon,T_atm,mCO2tot,mH2Otot,FMQ,frac_ext,q): 
    ...
    for i1p in range(1,n_run):
        ...
        # print('parameters',T,Pn,f_O2,mCO2tot,mH2Otot)
...
#%%
    ...
    # title_complete = ("results/updated_all_2/tstart" + t_string + "_gwater" + gwater_string + "_H2O" + h2o_string + "_CO2" + CO2_string + "_f_ext" + f_ext_string + "_f_volc" + f_volc_string + ".csv")
    ...
    # else:
    #     title_partial = ("partial_tstart" + t_string + "_gwater" + gwater_string + "_H2O" + h2o_string + "_CO2" + CO2_string + "_f_ext" + f_ext_string + "_f_volc" + f_volc_string + ".csv")
    ...
    #         title = title_partial
    #     else:
    #         title_magma = ("magmaocean_only/magmaonly_tstart" + t_string + 'water' + gwater_string + "_f_volc" + f_volc_string + '.csv')
...
#%%
    ...
    if "none" in title:
        ...
        if np.ndim(startup)>1:
            ...
            moles_H2O_init = startup[0:si1,15]
            moles_H2Oloss_init = startup[0:si1,16]
            moles_O2_init = startup[0:si1,17]
            Rmm_atm_init = startup[0:si1,18]
            T_atm_init = startup[0:si1,19]
            ash_init = startup[0:si1,20]
            lava_init = startup[0:si1,21]
            magma_init = startup[0:si1,22]
            nonthermal_init = startup[0:si1,23]
            z_meltlayer_tot_init = startup[0:si1,24]
            
            print('i1_init',i1_init,'size',np.size(i1_init))
            print('nan to num',np.nan_to_num(i1_init),'max',np.max(np.nan_to_num(i1_init)))
            ...
            moles_H2O_init = startup[15]
            moles_H2Oloss_init = startup[16]
            moles_O2_init = startup[17]
            Rmm_atm_init = startup[18]
            T_atm_init = startup[19]
            ash_init = startup[20]
            lava_init = startup[21]
            magma_init = startup[22]
            nonthermal_init = startup[23]
            z_meltlayer_tot_init = startup[24]
    ...
    if begin >= 0.99*nsteps:
        ...
        return(t_init,i1_init,Pres,moles_atm,Rmm_atm,moles_CO2,moles_H2O,moles_O2,dz,moles_H2Oloss,T_surf,moles_O2_ash,moles_O2_lava,moles_O2_magma,moles_O2_nt,z_meltlayer_tot)
    ...
    t,t_sun,Fxuv_venus,FH_ref,Fxuv_E = timestep(tstart,age_sun,nsteps,epsilon,magma_yn)

    if metamorphic_degas == 0:
        nn,m = eruption_rate(nsteps,moles_CO2[begin],frac_ext,P_final,mCO2tot,age_sun,tstart,b_err,dzdt_pres)
        dz_check = m*np.exp(-b_err*(t_sun - tstart)) + nn
   ...
   #%%     
    ...
    for i1 in range(begin,nsteps):
        ...
        if total_erupt > z_max:
            ...
            return(t_init,i1_init,Pres,moles_atm,Rmm_atm,moles_CO2,moles_H2O,moles_O2,dz,moles_H2Oloss,T_surf,moles_O2_ash,moles_O2_lava,moles_O2_magma,moles_O2_nt,z_meltlayer_tot,z_max)
            
            break
        
        # Calculate crustal production at current timestep
        dz[i1] = m*np.exp(-b_err*((t[i1-1]/secingyr) - tstart)) + nn
        ...
        # stop calculating depth of melt layer beyond 50000 km (sufficient to remove O2 from 1 Earth Ocean through oxidation, i.e. 3x max model H2O input)
        if z_meltlayer_tot[i1] >= 50000:
            ...
            nn,m = eruption_rate(nsteps,moles_CO2[i1],frac_ext,P_final,mCO2tot,age_sun,(t[i1]/secingyr),b_err,dzdt_pres)
        ...
        if z_melt[i1] > 0. and endmagma == 0.:
            ...
            moles_H2O[i1],moles_O2[i1],moles_H2Oloss[i1],tf,T_surf[i1],meltfrac0,moles_O2_ash[i1],moles_O2_lava[i1],dmoles_O2_magma,moles_O2_nt[i1],T_melt,latent,ox_frac0,z_meltlayer_tot[i1],endmagma = meltloss(t[i1-1],t[i1],moles_H2O[i1],moles_CO2[i1],
        ...
        if z_melt[i1] == 0. or endmagma == 1.:
            ...
            rk2_CO2,rk2_H2O,rk2_O2,rk2_Pres,rk2_moles_O2_ash,rk2_moles_O2_lava,rk2_moles_O2_nt = stdrk4(degas,frac_ext,b_err,FH_ref[i1],T,T_atm,FMQ,mCO2tot,mH2Otot,(moles_CO2[i1-1] + (0.5*rk1_CO2)),(moles_H2O[i1-1] + (0.5*rk1_H2O)),(moles_O2[i1-1] + (0.5*rk1_O2)),t[i1-1],thalf,tstart,dz[i1],sH2O,sCO2)
            rk3_CO2,rk3_H2O,rk3_O2,rk3_Pres,rk3_moles_O2_ash,rk3_moles_O2_lava,rk3_moles_O2_nt = stdrk4(degas,frac_ext,b_err,FH_ref[i1],T,T_atm,FMQ,mCO2tot,mH2Otot,(moles_CO2[i1-1] + (0.5*rk2_CO2)),(moles_H2O[i1-1] + (0.5*rk2_H2O)),(moles_O2[i1-1] + (0.5*rk2_O2)),t[i1-1],thalf,tstart,dz[i1],sH2O,sCO2)
            rk4_CO2,rk4_H2O,rk4_O2,rk4_Pres,rk4_moles_O2_ash,rk4_moles_O2_lava,rk4_moles_O2_nt = stdrk4(degas,frac_ext,b_err,FH_ref[i1],T,T_atm,FMQ,mCO2tot,mH2Otot,(moles_CO2[i1-1] + rk3_CO2),(moles_H2O[i1-1] + rk3_H2O),(moles_O2[i1-1] + rk3_O2),t[i1-1],t[i1],tstart,dz[i1],sH2O,sCO2)
            ...
            moles_O2_ash[i1] = moles_O2_ash[i1-1] + ((1/6) * (rk1_moles_O2_ash + (2*rk2_moles_O2_ash) + (2*rk3_moles_O2_ash) + rk4_moles_O2_ash))
            moles_O2_lava[i1] = moles_O2_lava[i1-1] + ((1/6) * (rk1_moles_O2_lava + (2*rk2_moles_O2_lava) + (2*rk3_moles_O2_lava) + rk4_moles_O2_lava))
        ...
        if t_diff >= threshold_time:
            ...
            return(t,i1,Pres,moles_atm,Rmm_atm,moles_CO2,moles_H2O,moles_O2,dz,moles_H2Oloss,T_surf,moles_O2_ash,moles_O2_lava,moles_O2_magma,moles_O2_nt,z_meltlayer_tot)
    ...
    return(t,i1,Pres,moles_atm,Rmm_atm,moles_CO2,moles_H2O,moles_O2,dz,moles_H2Oloss,T_surf,moles_O2_ash,moles_O2_lava,moles_O2_magma,moles_O2_nt,z_meltlayer_tot,z_max)
...
def timestep(tstart,t_end,nsteps,epsilon,magma):
    ...
    Fxuv_E = 1.6e30*((4.5/tau_sun)**(-beta)) * (1.e-7/A_venus) / (4*math.pi*((a_venus/AU)**2))
    
    # Reference H escape rate (from Tian 2015)
    FH_ref = (epsilon * (R_venus + 200.e3)**3 * Fxuv_venus)/(4*math.pi*G*M_venus)
    
    return(t,t_sun,Fxuv_venus,FH_ref,Fxuv_E)

#%%
def eruption_rate(nsteps,moles_CO2,frac_ext,P_final,mCO2tot,t_sun,tstart,b_err,dzdt_pres):
    ...
    nn = ((dzt_p*(1./b_err)*(1. - math.exp(-b_err*te))) - (z_lava*math.exp(-b_err*te))) / (((1./b_err)*(1. - math.exp(-b_err*te))) - (te*math.exp(-b_err*te)))
    m = (dzt_p - nn)/(math.exp(-b_err*te))
...
#%%
def degassing(Pres,T,FMQ,mCO2tot,mH2Otot,moles_CO20,moles_H2O0,t1,t2,dz,sH2O,sCO2):
    ...
    mol_CO2_degas = rho* dCO2/Rmm_CO2
    mol_H2O_degas = rho* dH2O/Rmm_H2O
    mol_tot_degas = mol_CO2_degas + mol_H2O_degas
    ...
    moles_CO2 = moles_CO20 + (dz_C*rho*mCO2tot)/Rmm_CO2
    moles_H2O = moles_H2O0 + (dz_C*rho*dH2O)/Rmm_H2O

    return(moles_CO2,moles_H2O,V_frac)

#%%
def Hloss(moles_H2O,moles_CO2,moles_O2,T_atm,FH_ref,t1,t2,kb,Rmm_atm):
    # Calculate XUV and diffusion limited H escape
    
    # Calculate background atmosphere properties
    moles_atm_heavy = 0.5*moles_H2O + moles_CO2 + moles_O2
    mean_matm_heavy = (Rmm_O2*(0.5*moles_H2O + moles_O2) + Rmm_CO2*moles_CO2)/moles_atm_heavy
    
    # Mass ratio of H and O for calculating whether O dragged along
    q = Rmm_O/Rmm_H
    
    # Mixing ratios of H and O
    mixing_rat_H = (2.*moles_H2O)/(moles_atm_heavy + (2.*moles_H2O))
    mixing_rat_O = (2.*moles_O2)/(moles_atm_heavy + (2.*moles_H2O))
    M_Oloss_column = 0.
    
    # Diffusion coefficient for H in O
    ...
    if mixing_rat_H>1.e-6:
        ...
        b_H_CO2 = 100 * (1.52e18) * math.sqrt(T_atm) * math.sqrt((0.001/Rmm_atm) + (0.001/Rmm_H))

        # Calculate diffusion limited escape
        M_Hloss_column_diff = (t2-t1) * (Rmm_H/Avo)* mixing_rat_H * ((G * M_venus )/(R_venus**2)) * ((mean_matm_heavy - Rmm_H)/Avo) * (b_H_CO2/(kb*T_atm))
    ...
    # H2O loss in moles 
    moles_H2Oloss = (M_Hloss_column/Rmm_H)*0.5 #equivalent moles of H2O lost through H escape
    moles_O2gain = ((M_Hloss_column/Rmm_H)*0.5*0.5) - 0.5*M_Oloss_column/Rmm_O #moles of O2 left behind by H escape
    ...
    return(moles_H2O,moles_O2,moles_H2Oloss)

#%%
def meltloss(t1,t2,moles_H2O0,moles_CO20,moles_O20,T_atm,kb,q,meltfrac0,epsilon,T,latent,ox_frac0,z_meltlayer_tot0,endmagma): #,rbfi,solidus,gill_t,gill_molesO2):
    ...
    t,t_sun,Fxuv_venus,FH_ref,Fxuv_E = timestep((t1/secingyr),(t2/secingyr),nsteps,epsilon,magma_yn)
    ...
    moles_atm = np.zeros(nsteps)
    moles_atm[0] = moles_CO2[0] + moles_H2O[0] + moles_O2[0]
    ...
    for i1 in range(1,nsteps):
        ...
        if T_surf[i1] < 1323.3:
            ...
            return(moles_H2O_out,moles_O2_out,moles_H2Oloss,t[i1],T_surf[i1],meltfrac_out,ash,lava,magma,nonthermal,T,latent,ox_frac,z_meltlayer_tot0,endmagma)
    ...
    return(moles_H2O_out,moles_O2_out,moles_H2Oloss,t[i1],T_surf[i1],meltfrac_out,ash,lava,magma,nonthermal,T,latent,ox_frac,z_meltlayer_tot,endmagma)
...
#%%
def nonthermal(t1,t2,moles_O2,Rmm_atm,T_atm,moles_H2O,moles_CO2,moles_atm):
    ...
    mean_matm_notO = (moles_CO2*Rmm_CO2 + moles_H2O*Rmm_H2O)/(moles_CO2 + moles_H2O)
...
#%%
def stdrk4(degas,frac_ext,b_err,FH_ref,T,T_atm,FMQ,mCO2tot,mH2Otot,moles_CO20,moles_H2O0,moles_O20,tstart,tend,tstart_model,dz,sH2O,sCO2):
    # Compute values for RK4 numerical integration
    
    # Initial atmosphere at beginning of timestep
    moles_atm0 = moles_CO20 + moles_H2O0 + moles_O20
    Rmm_atm0 = (moles_CO20*Rmm_CO2 + moles_H2O0*Rmm_H2O + moles_O20*Rmm_O2)/moles_atm0
    Pres0 = moles_atm0 * Rmm_atm0 * g

    if degas == 1:
        # Include volcanic degassing in contributions to atmospheric species during timestep
        moles_CO2,moles_H2O,V_frac = degassing(Pres0,T,FMQ,mCO2tot,mH2Otot,moles_CO20,moles_H2O0,tstart,tend,dz,sH2O,sCO2)
    ...
    # Update atmosphere after degassing
    moles_atm = moles_CO2 + moles_H2O + moles_O20
    Rmm_atm = (moles_CO2*Rmm_CO2 + moles_H2O*Rmm_H2O + moles_O20*Rmm_O2)/moles_atm
    
    # Find H loss and O2 accumulated
    moles_H2O,moles_O2,moles_H2Oloss = Hloss(moles_H2O,moles_CO2,moles_O20,T_atm,FH_ref,tstart,tend,kb,Rmm_atm)
    
    # Update atmosphere after H loss
    moles_atm = moles_CO2 + moles_H2O + moles_O20
    Rmm_atm = (moles_CO2*Rmm_CO2 + moles_H2O*Rmm_H2O + moles_O2*Rmm_O2)/moles_atm
    ...
    moles_O2_space = nonthermal(tstart,tend,moles_O2,Rmm_atm,T_atm,moles_H2O,moles_CO2,moles_atm)
    ...
    return(dmoles_CO2,dmoles_H2O,dmoles_O2,dPres,moles_O2_ash,moles_O2_lava,moles_O2_nt) #,dmoles_atm,dRmm_atm,dPres